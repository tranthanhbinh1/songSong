    ...
    public static DownloadConfig fromArgs(String[] args) {
        if (args.length < 1 || args.length > 6) {
            throw new IllegalArgumentException(
                    "Usage: <filename> [directoryHost=localhost] [directoryPort=1099] "
                            + "[outputDir=downloads] [chunkSizeBytes=1048576] [compressionEnabled=true]");
        }
        ...
        boolean compressionEnabled = args.length >= 6 ? Boolean.parseBoolean(args[5]) : true;
